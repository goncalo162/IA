...
def test_multiple_trips_start_and_capacity():
    ...
    assert len(veiculo.viagens) == 1, (
        f"Esperado 1 viagem ativa, obtido {len(veiculo.viagens)}. Estado: {_state_snapshot(veiculo, 'after p1')}"
    )
    ...
    assert len(veiculo.viagens) == 2, (
        f"Esperado 2 viagens ativas, obtido {len(veiculo.viagens)}. Estado: {_state_snapshot(veiculo, 'after p2')}"
    )
    ...
    assert not ok3, f"Terceira viagem nao deveria iniciar. Estado: {_state_snapshot(veiculo, 'start p3')}"
    assert veiculo.numero_passageiros == 5, (
...
def test_progress_updates_all_and_conclude_single():
    ...
    if concluidas:
        ...
        assert veiculo.numero_passageiros < pax_before, (
            f"Passageiros deveriam reduzir apos concluir uma viagem. Antes={pax_before}, Depois={veiculo.numero_passageiros}. Estado: {_state_snapshot(veiculo, 'after conclude 1')}"
        )
        remaining_active = any(v2.viagem_ativa for v2 in veiculo.viagens)
        todas_inativas = all(not v2.viagem_ativa for v2 in veiculo.viagens) if veiculo.viagens else False
        assert remaining_active or len(veiculo.viagens) == 0 or todas_inativas, (
            f"Esperado viagens ativas remanescentes, lista vazia, ou restantes inativas. Estado: {_state_snapshot(veiculo, 'after conclude check')}"
        )
...
def test_environment_lists_include_in_progress():
    ...
    assert veiculo in disp, (
        f"Veiculo deveria estar em disponiveis. Estado: {_state_snapshot(veiculo, 'env DISPONIVEL')}"
    )

    veiculo.estado = EstadoVeiculo.EM_ANDAMENTO
    rs = env.listar_veiculos_ridesharing()
    assert veiculo in rs, (
        f"Veiculo deveria estar em ridesharing. Estado: {_state_snapshot(veiculo, 'env EM_ANDAMENTO')}"
    )
