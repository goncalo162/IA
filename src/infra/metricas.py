...
class Metricas:
    ...
    def gerar_relatorio(self) -> str:
        ...
        relatorio.append(f"Taxa de atendimento: {self.taxa_atendimento():.2f}%")
        relatorio.append("")
        relatorio.append(f"Tempo médio de resposta: {self.tempo_resposta_medio():.2f} min")
        relatorio.append(f"Distância total percorrida: {self.distancia_total:.2f} km")
        relatorio.append(f"Custo total: €{self.custo_total:.2f}")
        relatorio.append(f"Custo médio por km: €{self.custo_medio_por_km():.3f}")
        relatorio.append("")
        relatorio.append(f"Emissões totais: {self.emissoes_totais:.2f} kg CO₂")
        relatorio.append(f"Emissões médias por km: {self.emissoes_medias_por_km():.3f} kg CO₂/km")
        relatorio.append("")
        relatorio.append(f"Taxa de ocupação da frota: {self.taxa_ocupacao():.2f}%")
        relatorio.append("=" * 60)
