    ...
    def update_active_trips(self, viagens_dict):
        ...
        for veiculo_id, veiculo in viagens_dict.items():
            if hasattr(veiculo, 'rota_viagem') and veiculo.rota_viagem:
                ...
                destino = veiculo.destino if hasattr(
                    veiculo, 'destino') else veiculo.rota_viagem[-1]
                progresso = f"{veiculo.progresso_viagem:.1%}" if hasattr(
                    veiculo, 'progresso_viagem') else "0%"
