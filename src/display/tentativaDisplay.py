...
"""
Display base interface.
Permite criar diferentes tipos de visualização (console, gráfico, etc.)
"""

class DisplayBase(ABC):
    """
    Interface base para componentes de display/visualização.
    Permite trocar facilmente entre diferentes tipos de output.
    """
...
class DisplayGrafico(DisplayBase):
    ...
    def __init__(self, frequencia_display: float = 10.0):
        ...
        self.intervalo_update = 1.0 / frequencia_display  # Tempo real entre redesenhos em segundos
    ...
    def _inicializar_plot(self):
        ...
        self.fig.canvas.manager.set_window_title('Simulação de Frota - Visualização em Tempo Real')
    ...
    # Interactions registration happens below inside a guarded import
        ...
        try:
            ...
            self.stats_fig.canvas.manager.set_window_title('Estatísticas da Simulação')
    ...
    def _desenhar_estatisticas(self):
        ...
        if self.stats_ax is not None:
            ...
            self.stats_ax.text(0.01, 0.99, stats_text, va='top', ha='left', fontsize=10, family='monospace')
            try:
